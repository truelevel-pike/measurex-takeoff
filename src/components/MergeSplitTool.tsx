...
export default function MergeSplitTool() {
  ...
  }, [getCoords, isMerge, isSplit, firstPolyId, splitPolyId, splitPts, merge, split, setTool, findPolygonAt]);
    ...
    >
      ...
      {isSplit && splitPts.length === 1 && (
        <svg className="absolute inset-0 w-full h-full pointer-events-none">
          <line x1={splitPts[0].x} y1={splitPts[0].y} x2={splitPts[0].x} y2={splitPts[0].y} stroke="#00d4ff" strokeWidth={2} />
        </svg>
      )}
    </div>
  );
