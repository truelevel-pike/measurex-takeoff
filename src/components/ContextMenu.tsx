...
export default function ContextMenu({ x, y, polygonId, onClose }: ContextMenuProps) {
  ...
  const handleDelete = () => { deletePolygon(polygonId); onClose(); };
  const handleCopy = () => {
    if (!polygon) return;
    addPolygon({
  ...
  return (
    <div
      style={{ position: 'absolute', top: y, left: x, zIndex: 50, background: '#fff', borderRadius: 8, boxShadow: '0 4px 16px rgba(0,0,0,0.15)', padding: 4, minWidth: 180 }}
    ...
    >
      ...
      {classifications.length > 0 && (
        <>
          ...
          {classifications.map((cls) => (
            <button key={cls.id} className="flex items-center gap-2 w-full px-3 py-1.5 text-sm hover:bg-neutral-100 rounded text-left" onClick={() => handleReclassify(cls.id)}>
      ...
      <button className="flex items-center gap-2 w-full px-3 py-1.5 text-sm hover:bg-neutral-100 rounded" onClick={() => { setSelectedPolygon(polygonId); onClose(); }}>
      ...
      <div className="border-top mt-1 pt-1">
        <button className="w-full px-3 py-1.5 text-sm text-neutral-400 hover:bg-neutral-100 rounded text-left" onClick={onClose}>Cancel</button>
      </div>
    </div>
  );
}
