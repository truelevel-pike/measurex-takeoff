...
export default function QuantitiesPanel({ showTakeoffSearch = false, onTakeoffSearchSelect, isLoading, onClassificationZoom }: QuantitiesPanelProps) {
  ...
  const handleToggleViewMode = useCallback((mode: 'summary' | 'detailed') => {
    setViewMode(mode);
    localStorage.setItem('mx-quantities-view-mode', mode);
    if (mode === 'detailed') {
  ...
  // Summary mode: compute top-10 classification IDs by primary metric
  const summaryTopIds = useMemo<Set<string> | null>(() => {
    if (viewMode !== 'summary' || filtered.length <= 10) return null;
    ...
    return new Set(ranked.slice(0, 10).map((c) => c.id));
  }, [viewMode, filtered, totalsByClassification]);

  const summaryExcludedCount = summaryTopIds ? filtered.length - 10 : 0;

  const summaryFilteredItems = useMemo<ListItem[]>(() => {
    if (!summaryTopIds) return orderedListItems;
    return orderedListItems.filter((item) => {
      if (item.kind === 'header') return true;
      return summaryTopIds.has(item.classification.id);
    }).filter((item, idx, arr) => {
      // Remove trade headers that have no rows after them
      if (item.kind !== 'header') return true;
      const next = arr[idx + 1];
      return next && next.kind === 'row';
    });
  }, [orderedListItems, summaryTopIds]);
      ...
      <div className="px-3 py-2 border-b border-[#00d4ff]/20 font-semibold text-[#e5e7eb] text-sm flex items-center justify-between bg-[rgba(10,10,15,0.6)] relative">
        ...
        <div className="flex items-center gap-2" suppressHydrationWarning>
          ...
          {filtered.length >= 3 && (<>
            ...
            <div className="flex items-center rounded overflow-hidden border border-gray-700/50 text-[10px]">
              <button
                type="button"
                onClick={() => handleToggleViewMode('summary')}
                className={`px-1.5 py-0.5 transition-colors ${viewMode === 'summary' ? 'text-[#00d4ff] bg-[#00d4ff]/10' : 'text-gray-400 hover:text-gray-200'}`}
                aria-label="Summary view"
                title="Show top 10 classifications"
              >
                Summary
              </button>
              <button
                ...
                className={`px-1.5 py-0.5 transition-colors ${viewMode === 'detailed' ? 'text-[#00d4ff] bg-[#00d4ff]/10' : 'text-gray-400 hover:text-gray-200'}`}
                aria-label="Detailed view"
                title="Show all classifications"
              >
                Detailed
              </button>
            </div>
              ...
              {isEditing ? (
                ...
                >
                  ...
                  {(() => {
                    ...
                    const confLabel = avgConf >= 0.85 ? 'high' : avgConf >= 0.70 ? 'medium' : 'low';
                    return (
                      <span className="text-[9px] px-1 rounded flex-shrink-0" style={{ backgroundColor: confColor, color: confTextColor, opacity: 0.85 }} title={`AI confidence: ${confPct}% ${confLabel}`}>{confPct}%</span>
                    );
                  })()}
